...
function takeCommand(message) {
    if (message.includes('hey') || message.includes('hello')) {
        ...
        window.open("https://wa.me/918839076135?call=true", "_blank");
        speak("Calling Aditya...");
    } else if (message.includes("call satyapal")) {
        ...
        const songName = message.replace("play song", "").trim(); // Extract song name from the command
        if (songName) {
            const searchUrl = `https://www.youtube.com/results?search_query=${songName}`;
            window.open(searchUrl, "_blank");
            speak(`Opening YouTube for the song: ${songName}`);
        } else {
